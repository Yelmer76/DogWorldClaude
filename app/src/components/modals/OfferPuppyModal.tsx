...
import { puppies, type Puppy } from "@/data/universe";
...
export function OfferPuppyModal({
  ...
  // Only puppies that aren't already reserved/beholdt/solgt are offerable
  const offerable = puppies.filter(
  ...
  function send() {
    const puppy = puppies.find((p) => p.id === pickedId);
    if (!puppy) return;
    showToast(
      `Tilbud sendt til ${applicantName}: ${puppy.name} (${puppy.colorLabel})`,
      "success",
    );
    handleClose();
  }
